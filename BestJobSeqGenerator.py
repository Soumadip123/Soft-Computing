...
def makespan_OR_tft(M,S):     #To calculate and return makespan or total flow time(tft) based on c's value   
...
sorted_ml = sorted(b, key=lambda x: x[mcs])

nu_sorted=[[0]*mcs for _ in range(jobs)]
for i in range(jobs):
    for j in range(mcs):
        nu_sorted[i][j]=sorted_ml[i][j]
# print(sorted_ml)
# print(nu_sorted)
mksp1 = makespan_OR_tft(nu_sorted,[0,1])
mksp2 = makespan_OR_tft(nu_sorted,[1,0])
best_seq=[]
nu_seq=[]
if mksp1< mksp2:
...
for i in range(2,jobs):
    ...
    for j in range(0,i+1):
        bs_copy=nu_seq[:]
        bs_copy.insert(j,i)
        mk_list.append(makespan_OR_tft(nu_sorted,bs_copy))
    min=mk_list[0]
    ind=0
